...
namespace pano {
    ...
    Vec2D ConnectedImages::get_final_resolution() const {
        LOGD("projmin: %f %f , projmax: %f %f \n", proj_range.min.x,proj_range.min.y,proj_range.max.x, proj_range.max.y);
        ...
        LOGD("Identity projection range: %f %f \n" , id_img_range.x, id_img_range.y);
        ...
        LOGD("Target Image Size: (%lf, %lf)\n", target_size.x, target_size.y);
        if (max_edge > 80000 || target_size.x * target_size.y > 1e9){
            LOGD("Target size too large. Looks like a stitching failure!\n");
            abort();
        }
        // resize the result
