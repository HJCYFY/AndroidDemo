...
namespace pano {
    ...
    Mat32f Stitcher::build() {
        ...
        LOGD("Using projection method: %d\n", bundle.proj_method);
    ...
    bool Stitcher::match_image(
        ...
        if (!succ) {
            if (-(int)info.confidence >= 8)	// reject for geometry reason
                LOGD("Reject bad match with %d inlier from %d to %d\n",
...
#pragma omp parallel for schedule(dynamic)
        ...
        LOGD("Total number of matched keypoint pairs: %d\n", total_nr_match);
    }
...
#pragma omp parallel for schedule(dynamic)
        REP(i, n) {
            ...
            if (!match_image(pwmatcher, i, next)) {
                ...
                {
                    LOGD("Image %d and %d don't match\n", i, next);
                    abort();
                }
            }
            continue;
